...
import { LuArrowLeft, LuZap, LuCheck, LuCircleX, LuImage, LuArrowRight, LuLoader, LuFileText } from "react-icons/lu";
...
export default function AssetsPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([projData, scriptData]) => {
        ...
        // 使用第一个脚本（用户在脚本页选择的脚本），后续可扩展为传递所选脚本index
        if (Array.isArray(scriptData) && scriptData.length > 0) {
          const script = scriptData[0];
          const shots: AssetItem[] = (script.shots ?? []).map((shot: any) => ({
            shotId: shot.shotId,
            type: shot.type,
            ...
            status: (shot.visualSource === "product_image" ? "done" : "pending") as AssetItem["status"],
            thumbnailUrl: "",
          }));
          setAssets(shots);
        } else {
  ...
  const generateOne = useCallback((shotId: number) => {
    ...
    // 模拟 2-4 秒延迟（实际接入 AI 生图 API 时替换此处）
    ...
    return (
      <div className="min-h-screen grid-bg flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-sm text-muted-foreground">正在加载素材数据...</span>
        </div>
      </div>
    );
  }
        ...
        {assets.length === 0 ? (
          ...
          <>
            ...
            <div className="space-y-4">
              {assets.map((asset) => {
                const typeInfo = shotTypeLabels[asset.type] ?? { label: asset.type, color: "bg-zinc-500/20 text-zinc-400" };
                return (
